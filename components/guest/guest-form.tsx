...
import {Modal, ModalHeader, ModalFooter, ModalContent, ModalBody, Button, Input, Textarea, Select, SelectItem, Checkbox} from '@nextui-org/react'
...
export default function GuestForm({isOpen, onOpenChange, userId, guest}: GuestFormProps) {
    ...
    const firstName = useInput(GuestCreateInput.pick({firstName: true}), 'firstName', guest?.firstName ? guest.firstName : '')
    const lastName = useInput(GuestCreateInput.pick({lastName: true}), 'lastName', guest?.lastName ? guest.lastName : '')
    const foodProhibitions = useInput(GuestCreateInput.pick({foodProhibitions: true}), 'foodProhibitions', guest?.foodProhibitions ? guest.foodProhibitions : '')
                        ...
                        <ModalBody>
                            <div className="flex flex-col md:flex-row md:flex-wrap">
                                <div className="flex flex-col md:w-1/2 md:mr-4">
                                    ...
                                    {isChild && <Select name="menu" label="Menu" value={guest?.menu} isRequired={true}
			                                            selectedKeys={menuValue} onSelectionChange={setMenuValue}>
